...
void MenuManager::OnMove(olc::PixelGameEngine& GameEngine) {
	...
	if (GameEngine.GetKey(olc::ESCAPE).bPressed || GameEngine.GetKey(olc::X).bPressed) {
	...
	// Move though menu
	if (GameEngine.GetKey(olc::W).bPressed || GameEngine.GetKey(olc::K).bPressed) stMenu.back()->OnMove({  0, -1  });
	if (GameEngine.GetKey(olc::S).bPressed || GameEngine.GetKey(olc::J).bPressed) stMenu.back()->OnMove({  0,  1  });
	if (GameEngine.GetKey(olc::A).bPressed || GameEngine.GetKey(olc::H).bPressed) stMenu.back()->OnMove({ -1,  0  });
	if (GameEngine.GetKey(olc::D).bPressed || GameEngine.GetKey(olc::L).bPressed) stMenu.back()->OnMove({  1,  0  });
	if (GameEngine.GetKey(olc::ENTER).bPressed || GameEngine.GetKey(olc::Z).bPressed) OnConfirm();
}
