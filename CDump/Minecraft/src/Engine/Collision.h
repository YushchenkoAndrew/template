...
public:
	...
	template <class T>
	static inline bool RectangleCollition(const T& v1Pos, const T& v2Pos, const T& v1Size, const T& v2Size) {
		return
			(v1Pos.x + v1Size.x) >= v2Pos.x &&
			(v2Pos.x + v2Size.x) >= v1Pos.x &&
			(v1Pos.y + v1Size.y) >= v2Pos.y &&
			(v2Pos.y + v2Size.y) >= v1Pos.y;
	}
	...
	template <class T>
	static bool DiagonalStaticRectIntesection(T& v1Pos, T& v2Pos, const T& v1Size, const T& v2Size) {
		const T* pv1Pos = &v1Pos; const T* pv1Size = &v1Size;
		const T* pv2Pos = &v2Pos; const T* pv2Size = &v2Size;
		...
		for (int32_t k = 0; k < 2; k++) {
			...
			for (int32_t i = 0; i < 4; i++) {
				...
				for (int32_t j = 0; j < 4; j++) {
					...
					// FIXME: Problem with static collision
...
struct sRectanleCollision {
	...
	static inline bool IsCollide(const T& v1Pos, const T& v2Pos, const T& v1Size, const T& v2Size) {
		return Collision::RectangleCollition(v1Pos, v2Pos, v1Size, v2Size);
	}
};
...
struct sDiagonalCollision {
	template<class T>
	static inline bool IsCollide(const T& v1Pos, const T& v2Pos, const T& v1Size, const T& v2Size) {
		return Collision::DiagonalRectIntesection(v1Pos, v2Pos, v1Size, v2Size);
	}
};
...
struct sDiagonalStaticCollision {
	...
	static inline bool IsCollide(T& v1Pos, T& v2Pos, const T& v1Size, const T& v2Size) {
		return Collision::DiagonalStaticRectIntesection(v1Pos, v2Pos, v1Size, v2Size);
	}
};
